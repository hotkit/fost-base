...
namespace {
    ...
    const fostlib::setting< int > c_count( L"FizzBuzz main.cpp", L"FizzBuzz", L"Count", 100, true );
...
FSL_MAIN(
    ...
    boost::scoped_ptr< fostlib::setting< int > > count;
    if ( args.size() == 2 )
        count.reset( new fostlib::setting< int >( L"Command line", c_count, fostlib::coerce< int >( args[ 1 ] ) ) );
    fizzbuzz( out );
    return 0;
}
