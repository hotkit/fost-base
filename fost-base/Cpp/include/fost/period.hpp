...
namespace fostlib {
    ...
    /// Allows us to store a controlled value which is only valid between two timestamps
    template< typename V > class period;
...
namespace std {
    ...
    inline fostlib::ostream &operator << (
        fostlib::ostream &o, const fostlib::period< V > &p
    ) {
