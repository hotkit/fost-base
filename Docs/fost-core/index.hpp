/** \defgroup fost_core fost-core
    ...
    - \ref fost_core_json -- A JSON parser and printer together with suitable data types to handle JSON objects, arrays and atoms.
    - \ref fost_core_coerce -- A type safe type coercion library which is extensible for user defined types.
    - \ref fost_core_threading -- A threading library providing futures and worker threads with thread affinity.
