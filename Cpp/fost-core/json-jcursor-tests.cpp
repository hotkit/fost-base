...
FSL_TEST_FUNCTION(empty) {
    ...
    FSL_CHECK_EQ(
            fostlib::json(L"Hello")[fostlib::jcursor()],
...
FSL_TEST_FUNCTION(json_position) {
    ...
    FSL_CHECK_EQ(
            a[fostlib::jcursor(fostlib::json(L"key"))], fostlib::json(123));
...
FSL_TEST_FUNCTION(updates) {
    ...
    FSL_CHECK_EXCEPTION(
            (fostlib::jcursor(L"key") / L"1st")(j),
            fostlib::exceptions::json_error &);
    FSL_CHECK_NOTHROW(
            (fostlib::jcursor(L"key2") / L"1st")(j) =
                    fostlib::json(L"value 1"));
    FSL_CHECK_NOTHROW(
            (fostlib::jcursor(L"key2") / L"2nd")(j) =
