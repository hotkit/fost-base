...
timestamp fostlib::coercer< timestamp, rfc1123_timestamp >::coerce(
    ...
    if ( boost::spirit::qi::phrase_parse(pos, end, rule, boost::spirit::qi::space, ret) && pos == end ) {
        return ret;
    } else {
        throw exceptions::parse_error("Parsing an RFC1123 time stamp", string(pos, end));
    }
}
