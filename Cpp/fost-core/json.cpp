...
bool fostlib::json::isnull() const {
    return std::visit( ::isnull(), m_element );
}
...
bool fostlib::json::isatom() const {
    return std::visit( ::isatom(), m_element );
}
...
namespace {
    struct comparator {
        ...
        bool operator ()( const json::array_p &ta ) const {
            if ( !std::get_if<json::array_p>(&r) ) return false;
            else {
...
namespace {
    struct size_finder {
        json::array_t::size_type operator () (std::monostate) const {
            return 0;
        }
        template<typename T>
...
bool fostlib::json::has_key( const jcursor &p ) const {
    if ( p.m_position.empty() )
        ...
        return std::visit(::path_has_key(*this, tail, tail.m_position.size()),
            *( p.m_position.begin()));
    }
}
...
const json &fostlib::json::operator[]( const jcursor &p ) const {
    if ( p.m_position.empty() )
        ...
        return std::visit( ::path_walker( *this, jcursor( ++( p.m_position.begin() ), p.m_position.end() ) ), *( p.m_position.begin() ) );
}
