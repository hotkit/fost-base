...
void fostlib::base16_string_tag::do_encode(const ascii_string &l, ascii_string &s) {
    s = l;
    check_encoded(s);
}
...
std::vector<unsigned char> fostlib::coercer<std::vector<unsigned char>, fostlib::base16_string>::coerce(
    const base16_string &s
) {
    if ( s.length() & 1 ) throw fostlib::exceptions::parse_error(
