...
fostlib::yaml::record &
        fostlib::yaml::record::add(const fostlib::string &k, fostlib::t_null) {
    ::push_back(content, k + ": ~");
    return *this;
}
...
fostlib::yaml::record &fostlib::yaml::record::add(
    ...
    if (&v == this)
        throw fostlib::exceptions::not_implemented(
                "fostlib::yaml::record::add -- recursively adding a record to "
                "itself");
    ::push_back(content, k + ": [");
    for (collection_type::const_iterator i = v.content.begin();
         i != v.content.end(); ++i)
