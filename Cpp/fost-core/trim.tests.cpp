...
FSL_TEST_FUNCTION(basic_trim) {
    ...
    FSL_CHECK_EQ(fostlib::trim(fostlib::nullable<fostlib::ascii_string>{" string "}), "string");
    FSL_CHECK_EQ(fostlib::trim(fostlib::nullable<fostlib::utf8_string>{"\xE2\x80\x82\xE2\x80\x83\xE2\x80\x84string\x20"}), "string");
    FSL_CHECK_EQ(fostlib::trim(fostlib::nullable<fostlib::string>{"\xE2\x80\x82\xE2\x80\x83\xE2\x80\x84string\x20"}), "string");
}

FSL_TEST_FUNCTION(specific_trim) {
    FSL_CHECK_EQ(TRIM_WITH("_string_", "_"), "string");
    FSL_CHECK_EQ(TRIM_WITH(, ""), fostlib::null);
    FSL_CHECK_EQ(TRIM_WITH("\x20\xE2\x80\x8Astring", "\x20"), "\xE2\x80\x8Astring");
    FSL_CHECK_EQ(fostlib::trim(fostlib::string{"xystringyx"}, fostlib::string{"xy"}), "string");
}