// tweetnacl.cpp - modified tweetnacl.c placed in public domain by Jeffrey
// Walton.
...
// Further modified for use in Fost libraries
...
static const uint8_t _0[32] = {0}, _9[32] = {9};

static const gf gf0 = {0}, gf1 = {1}, _121665 = {0xDB41, 1},
                D = {0x78a3, 0x1359, 0x4dca, 0x75eb, 0xd8ab, 0x4141,
                     0x0a4d, 0x0070, 0xe898, 0x7779, 0x4079, 0x8cc7,
                     0xfe73, 0x2b6f, 0x6cee, 0x5203},
                D2 = {0xf159, 0x26b2, 0x9b94, 0xebd6, 0xb156, 0x8283,
                      0x149a, 0x00e0, 0xd130, 0xeef3, 0x80f2, 0x198e,
                      0xfce7, 0x56df, 0xd9dc, 0x2406},
                X = {0xd51a, 0x8f25, 0x2d60, 0xc956, 0xa7b2, 0x9525,
                     0xc760, 0x692c, 0xdc5c, 0xfdd6, 0xe231, 0xc0a4,
                     0x53fe, 0xcd6e, 0x36d3, 0x2169},
                Y = {0x6658, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666,
                     0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666,
                     0x6666, 0x6666, 0x6666, 0x6666},
                I = {0xa0b0, 0x4a0e, 0x1b27, 0xc4ee, 0xe478, 0xad2f,
...
int crypto_core_salsa20(
        uint8_t *out, const uint8_t *in, const uint8_t *k, const uint8_t *c) {
    core(out, in, k, c, 0);
    return 0;
}
...
static const uint8_t sigma[16] = {0x65, 0x78, 0x70, 0x61, 0x6E, 0x64,
                                  0x20, 0x33, 0x32, 0x2D, 0x62, 0x79,
                                  0x74, 0x65, 0x20, 0x6B};
...
int crypto_stream_salsa20(
        uint8_t *c, uint64_t d, const uint8_t *n, const uint8_t *k) {
...
int crypto_onetimeauth(
        uint8_t *out, const uint8_t *m, uint64_t n, const uint8_t *k) {
...
int crypto_onetimeauth_verify(
        const uint8_t *h, const uint8_t *m, uint64_t n, const uint8_t *k) {
    uint8_t x[16];
...
static int has_small_order(const uint8_t s[32]) {
    ...
    unsigned int k = 0;
    for (size_t i = 0; i < COUNTOF(blacklist); i++) { k |= (c[i] - 1); }
...
int crypto_box_beforenm_unchecked(
        uint8_t *k, const uint8_t *y, const uint8_t *x) {
    uint8_t s[32];
    if (crypto_scalarmult(s, x, y) != 0) return -1;
    return crypto_core_hsalsa20(k, _0, s, sigma);
}
