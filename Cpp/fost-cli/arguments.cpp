...
ostream &fostlib::arguments::printOn( ostream &o ) const {
    for ( std::map< string, string >::const_iterator it( m_environment.begin() ); it != m_environment.end(); ++it ) {
        o << L"argument,environment," << (*it).first << L"," << (*it).second << std::endl;
    }
    for ( std::vector< string >::const_iterator it( m_arguments.begin() ); it != m_arguments.end(); ++it ) {
        o << L"argument,command line argument," << (*it) << std::endl;
    }
    for ( std::map< string, string >::const_iterator it( m_switches.begin() ); it != m_switches.end(); ++it ) {
        o << L"argument,command line switch," << (*it).first << L"," << (*it).second << std::endl;
    }
    return o;
}
