...
const wchar_t * const fostlib::exceptions::invalid_ini_line::message() const throw () {
    ...
    for ( fostlib::string::const_iterator pc( file.begin() ); pc != file.end(); ++pc ) {
        ...
        if ( c == L'\n' || c == L'\r' ) {
            ...
            if ( line == L"" ) {
                ...
                if ( section == L"" || line.find( L']' ) == string::npos || section.find_first_of(  L"[]" ) != string::npos )
                ...
                fostlib::string name( line.substr( 0, pos ) ), value( line.substr( pos + 1 ) );
                json js = json::parse( value, json() );
                if ( !value.empty() && js.isnull() )
                    m_settings.push_back( boost::shared_ptr< setting< string > >( new setting< string >( m_fileName, section, name, value, false ) ) );
                else
                    m_settings.push_back( boost::shared_ptr< setting< json > >( new setting< json >( m_fileName, section, name, js, false ) ) );
            }
            line = L"";
        } else
            line += c;
