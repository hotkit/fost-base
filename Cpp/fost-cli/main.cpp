...
int fostlib::main_exec( const fostlib::ini_settings &settings, ostream &out, arguments &args, int (*main_f)( fostlib::ostream &, fostlib::arguments & ) ) {
    ...
    try {
        ...
        out << std::endl << L"Fost exception" << std::endl << e << std::endl;
    } catch ( std::exception &e ) {
        out << std::endl << L"std::exception" << std::endl << fostlib::string( e.what() ) << std::endl;
    } catch ( ... ) {
        out << std::endl << L"Unknown exception caught" << std::endl;
    }
    return 2;
}
