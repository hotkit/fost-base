...
FSL_TEST_FUNCTION( in_proc_constructor ) {
    ...
    fostlib::in_process<executor> ipc1([&count]() {
...
FSL_TEST_FUNCTION( in_proc_futures ) {
    ...
    FSL_CHECK_EQ(1, ipc.synchronous<int>([](auto &e) {
        return e.value();
    }));
    FSL_CHECK_EQ(1, ipc.asynchronous<int>([](auto &e) {
        return e.value();
    })());
}
...
FSL_TEST_FUNCTION( in_proc_void_future ) {
    ...
    FSL_CHECK_EQ(2, ipc.synchronous<int>([](auto &e) {
        return e.increment();
    }));
    ipc.synchronous<void>([](auto &e) {
        return e.increment();
    });
    FSL_CHECK_EQ(3, ipc.synchronous<int>([](auto &e) {
        return e.value();
    }));
