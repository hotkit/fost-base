...
: m_host( h ), user( u ), password( pw ), m_pathspec( L"/" ) {
}
...
string fostlib::url::as_string( const url &relative_from ) const {
	if ( g_allow_relative.value() &&
        ( host().service() == L"http" || host().service() == L"https" ) &&
        relative_from.host().service() == host().service() &&
        relative_from.host().name() == host().name()
    )
...
fostlib::host fostlib::url::host() const {
	return m_host;
}
...
fostlib::exceptions::relative_path_error::relative_path_error( const string &base, const string &rel, const string &error ) {
